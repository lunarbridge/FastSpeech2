...
def work(input_path, input_wav_path, txt_file, output_path, cleaners, sampling_rate, max_wav_value):
    # get base name
    ...
    # write wavefile
    wavfile.write(
        os.path.join(output_path, speaker, "{}.wav".format(base_name)),
        sampling_rate,
        wav.astype(np.int16),
    )
    ...
    # write textfile
    with open(
        os.path.join(output_path, speaker, "{}.lab".format(base_name)),
        "w",
    ) as f1:
        f1.write(text)


def prepare_align(data_relpaths: List, input_path: str, output_path: str,
    ...
    Parallel(n_jobs=cpu_count() - 1)(
        delayed(work)
        (input_path, wav_path, txt_file, output_path, cleaners, sample_rate, max_wav_value)
